...
class ContactService:
    ...
    def create_company(workspace_id: int, data: Dict[str, Any], user_id: int) -> Company:
        ...
        # Get max display_order for workspace and set new company to end of list
        max_order = db.session.query(db.func.max(Company.display_order)).filter_by(
    ...
    def create_contact(workspace_id: int, data: Dict[str, Any], user_id: int) -> Contact:
        ...
        # Get max display_order for workspace and set new contact to end of list
        max_order = db.session.query(db.func.max(Contact.display_order)).filter_by(
            workspace_id=workspace_id,
            is_deleted=False
        ).scalar() or -1
